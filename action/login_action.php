...
function login()
{
	...
	$username1 = str_replace(["-", "–"], '', $_POST['username']);
	$password1 = mysqli_real_escape_string($con, md5($password));
	// $password = md5($con,$password);


	$input = "$password";
	// include('md5.php');
	// SQL query to fetch information of registerd users and finds user match.
	$query = mysqli_query($con, "SELECT * FROM `user_account` WHERE `user_name` = '$username1' AND `user_password` = '$password1'");

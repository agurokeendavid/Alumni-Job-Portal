...
$sql = "SELECT usd.student_IDNumber, tracer.campus, cc.course_acronym, CONCAT(usd.student_lName, ', ', usd.student_fName, ' ', UPPER(SUBSTRING(usd.student_mName, 1, 1)), '.') AS fullname, usd.student_year_grad, tracer.date_hired_current_job, tracer.status_employment, tracer.monthly_income, tracer.percentage_increase, tracer.employer FROM user_student_detail usd LEFT JOIN capsu_course cc ON usd.student_course = cc.course_ID LEFT JOIN student_tracer_detail tracer ON usd.studentIDNumber = tracer.student_ID;";
...
$sql = "SELECT usd.student_IDNumber, tracer.campus, cc.course_acronym, CONCAT(usd.student_lName, ', ', usd.student_fName, ' ', UPPER(SUBSTRING(usd.student_mName, 1, 1)), '.') AS fullname, usd.student_year_grad, tracer.date_hired_current_job, tracer.status_employment, tracer.monthly_income, tracer.percentage_increase, tracer.employer FROM user_student_detail usd INNER JOIN capsu_course cc ON usd.student_course = cc.course_ID INNER JOIN student_tracer_detail tracer ON usd.studentIDNumber = tracer.student_ID;";
...
// }
$query=mysqli_query($con, $sql) or die("serverside_data_registeredtracer.php: get tracer");
